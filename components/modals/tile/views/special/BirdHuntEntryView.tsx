...
export const BirdHuntEntryView: React.FC<Props> = ({ state, dispatch, t, currentPlayer, close }) => {
    ...
    return (
        <div className="bg-[#1f2937] text-white w-full max-w-sm rounded-xl overflow-hidden shadow-[0_0_50px_rgba(34,197,94,0.5)] border-4 border-emerald-600 animate-in zoom-in-95 relative max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            {/* Header */}
            <div className="h-32 relative flex items-center justify-center bg-gradient-to-br from-emerald-900 to-green-950 overflow-hidden border-b-4 border-emerald-600 shrink-0">
                <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/nature.png')] opacity-30"></div>
                <div className="z-10 text-center">
                    <div className="text-6xl mb-2 filter drop-shadow-lg">🦆</div>
                    <h2 className="text-2xl font-black uppercase italic tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-emerald-300 to-white">
                        {t.name}
                    </h2>
                </div>
            ...
            <div className="p-6 bg-slate-900 relative space-y-4">
                ...
                {/* Special Action: Bird Hunt */}
                {isAtLocation ? (
                    <button 
                        onClick={() => dispatch({type: 'START_BIRD_HUNT'})} 
                        className="w-full bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-500 hover:to-emerald-400 text-white font-black py-4 rounded-lg shadow-lg border-b-4 border-green-800 active:scale-95 transition-all text-lg flex items-center justify-center gap-2 group"
                    >
                        <span>JUGAR DUCK HUNT</span>
                    ...
                    <div className="text-center text-xs text-gray-500 italic py-2 border border-slate-700 rounded bg-slate-950">
                        Debes estar aquí para iniciar la temporada de caza.
                    </div>
                )}
                    ...
                    <table className="w-full">
                        <tbody className="divide-y divide-slate-700/50">
                            {getRentTable(t).map((row, idx) => {
                                ...
                                return ( 
                                    <tr key={idx} className={isActive ? 'bg-emerald-900/50 text-emerald-300 font-bold' : 'text-gray-400'}>
                ...
                <button onClick={close} className="w-full mt-2 text-center text-xs text-emerald-400 hover:text-white underline">
                    Cerrar
                </button>
            </div>
        </div>
