...
export const FioreModal: React.FC<FioreModalProps> = ({ state, dispatch, t, currentPlayer }) => {
    ...
    return (
        <div className="bg-[#1a0b2e] w-full max-w-lg rounded-xl overflow-hidden shadow-[0_0_50px_#d946ef] border-4 border-fuchsia-600 animate-in zoom-in-95 relative" onClick={e => e.stopPropagation()}>
            ...
            {isClosed ? (
                <div className="p-10 text-center bg-black/90 relative z-10 h-64 flex flex-col items-center justify-center">
                    <div className="text-6xl mb-4 grayscale opacity-50">👠</div>
                    <h3 className="text-2xl font-black text-rose-500 uppercase tracking-widest border-4 border-rose-500 p-2 transform rotate-6 mb-4">
                        CLAUSURADO
                    </h3>
                    <p className="text-xs text-gray-300 max-w-xs leading-relaxed">
                        Este establecimiento ha sido cerrado por la Ley de Moralidad Pública del Gobierno de Izquierdas. 
                        <br/>
                        <span className="text-rose-400 font-bold mt-2 block">"La mercantilización del cuerpo está prohibida."</span>
                    </p>
                    <button onClick={() => dispatch({type: 'CLOSE_MODAL'})} className="mt-8 text-gray-500 hover:text-white underline text-xs">Salir</button>
                </div>
            ) : (
