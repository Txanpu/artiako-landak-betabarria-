...
interface Props {
    ...
    config: TileConfig;
    textRot: string;
    flexClass?: string; // Optional because logic handles default
}

export const SpecialRenderer: React.FC<Props> = ({ tile, config, textRot, flexClass = 'flex-col' }) => {
    
    // QUIZ (Always on Diagonals, Square-ish)
    ...
    // --- CASINO TILES (Can be side or diagonal) ---
    if (tile.subtype === 'casino_bj' || tile.subtype === 'casino_roulette') {
        const isRoulette = tile.subtype === 'casino_roulette';
        // Use flexClass for layout on sides
        return (
            <div className={`absolute inset-0 flex ${flexClass} items-center justify-center overflow-hidden border-4 border-yellow-700 bg-green-900`}>
                ...
                <div className={`z-10 text-[7px] font-black text-yellow-400 uppercase tracking-widest bg-black/60 px-2 rounded-full border border-yellow-600 mb-1 shadow-lg ${config.isDiagonal ? '' : textRot}`}>
                    CASINO
                </div>
                
                <div className={`z-10 text-3xl filter drop-shadow-lg transform hover:scale-110 transition-transform ${config.isDiagonal ? '' : textRot}`}>
                    {isRoulette ? '🎡' : '♠️'}
                </div>
                
                <div className={`z-10 text-[8px] font-black text-white text-center leading-tight mt-1 px-1 drop-shadow-md uppercase font-serif tracking-wide ${config.isDiagonal ? '' : textRot}`}>
    ...
    // --- CORNER TILES (Square, rotation OK) ---
    ...
    if (tile.type === TileType.START) {
        return (
            <div className={`absolute inset-0 flex flex-col items-center justify-center ${config.isDiagonal ? '' : textRot}`}>
                <div className="text-5xl z-10 drop-shadow-lg">🚩</div>
                <div className="text-[9px] font-black text-emerald-300 uppercase tracking-widest bg-black/50 px-1 rounded mt-1 border border-emerald-500">SALIDA</div>
            </div>
        );
    }
    ...
    // --- RECTANGULAR SIDE TILES (Use flexClass) ---

    if (tile.type === TileType.BANK) {
        return (
            <div className={`absolute inset-0 flex ${flexClass} items-center justify-center overflow-hidden`}>
                <div className="absolute inset-0 bg-slate-900"></div>
                <div className="absolute inset-0 bg-gradient-to-b from-yellow-900/20 to-black"></div>
                <div className={`text-4xl z-10 text-yellow-500 drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] ${config.isDiagonal ? '' : textRot}`}>🏦</div>
                <div className={`text-[8px] font-black text-yellow-500 uppercase tracking-widest mt-1 z-10 border-b border-yellow-700 ${config.isDiagonal ? '' : textRot}`}>BANCA</div>
            </div>
        );
    }
    ...
    if (tile.type === TileType.SLOTS) {
        return (
            <div className={`absolute inset-0 flex ${flexClass} items-center justify-center overflow-hidden`}>
                ...
                <div className={`text-4xl z-10 animate-pulse drop-shadow-[0_0_10px_rgba(255,0,255,0.8)] ${config.isDiagonal ? '' : textRot}`}>🎰</div>
                <div className={`text-[8px] font-black text-fuchsia-300 uppercase tracking-widest mt-1 z-10 border-b border-fuchsia-500/50 ${config.isDiagonal ? '' : textRot}`}>SLOTS</div>
    ...
    if (tile.subtype === 'greyhound') {
        return (
            <div className={`absolute inset-0 flex ${flexClass} items-center justify-center overflow-hidden bg-[#3d2b1f]`}>
                ...
                <div className={`text-4xl z-10 drop-shadow-md transform ${config.isDiagonal ? '-scale-x-100' : `${textRot} -scale-x-100`}`}>🐕</div>
                
                <div className={`text-[7px] font-black text-yellow-100 uppercase tracking-tighter mt-1 z-10 bg-yellow-900/80 px-2 py-0.5 rounded-full border border-yellow-500 shadow-sm whitespace-nowrap ${config.isDiagonal ? '' : textRot}`}>
                    🏁 CANÓDROMO
                </div>
            </div>
        );
    ...
    if (tile.name === 'Suerte') {
        return (
            <div className={`absolute inset-0 flex ${flexClass} items-center justify-center`}>
                <div className={`text-5xl font-serif text-orange-200/20 font-black absolute ${config.isDiagonal ? '' : textRot}`}>?</div>
                <div className={`text-orange-100 font-bold text-[10px] uppercase border-2 border-orange-300/50 px-1 py-0.5 rounded z-10 ${config.isDiagonal ? '' : textRot}`}>Suerte</div>
            </div>
        );
    }
    ...
    if (tile.name === 'Caja de Comunidad' || tile.name.includes('Comunidad')) {
        return (
            <div className={`absolute inset-0 flex ${flexClass} items-center justify-center`}>
                <div className={`text-4xl absolute opacity-20 ${config.isDiagonal ? '' : textRot}`}>📦</div>
                <div className={`text-center z-10 ${config.isDiagonal ? '' : textRot}`}>
    ...
    if (tile.type === TileType.TAX) {
        return (
            <div className={`absolute inset-0 flex ${flexClass} items-center justify-center`}>
                <div className={`text-4xl z-10 opacity-20 ${config.isDiagonal ? '' : textRot}`}>💸</div>
                <div className={`z-10 text-center ${config.isDiagonal ? '' : textRot}`}>
                    <div className="text-[9px] font-black text-red-400 uppercase tracking-widest border-b border-red-500/50">IMPUESTOS</div>
                </div>
            </div>
        );
    }
